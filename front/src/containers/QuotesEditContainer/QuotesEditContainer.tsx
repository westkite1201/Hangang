...
import { GET_QUOTES, EDIT_QUOTES, Quotes,Quote } from '../../lib/graphql/admin'
...
const QuotesCardList: React.FC<QuotesCardProps> = ({ quotes, setQuote, handleQuote ,quoteInfo}) => {
  const quotesList=  quotes.map((quote :Quote) => {
    return (
    <QuoteItem onClick = {() => setQuote(quote)} key ={quote._id}>
        {quote._id === quoteInfo.id ? <input value ={quoteInfo.word} onChange={(e) => handleQuote(e, quote)} name = 'word' /> : quote.WORD}
      <div>

        {quote._id === quoteInfo.id ? <input value ={quoteInfo.name} onChange={(e) => handleQuote(e, quote)} name = 'name'/> : quote.NAME}
      </div>
    </QuoteItem>
    )
  });
...
const QuotesGridViewComponent = () => {
  ...
  const handleQuote=(e :React.ChangeEvent<HTMLInputElement>, quote: Quote)=>{
      ...
      <Grid container spacing={3}>
        ...
        <Grid item xs={10} md={10} lg={10}>
          ...
          <button onClick = {handleEdit}>에딧 컨텐츠</button>
            {(data && data.quotes) && 
              <QuotesCardList quotes ={data.quotes} 
                              setQuote = {setQuote}
