...
import { getQuotesThunk, IQuotesData } from '../../lib/slices/quotesSlice';
import { RootState } from '../../store';
import { IQuote } from '../../interfaces';
import QuotesPreview from '../Quotes/QuotesPreview';
...
const QuoteList: React.FC<QuotesListProps> = ({ quotes, setting }) => {
  ...
  const quotesList = quotes.map((quote: IQuote, index: number) => {
    return (
      <Quote quote={quote} index={index} setting={setting} key={quote.id} />
    );
  });
  return <Fragment>{quotesList}</Fragment>;
};

function addId(quotes: IQuote[]) {
  return quotes.reduce((acc: IQuote[], curr: IQuote) => {
    acc.push({ ...curr, id: curr._id });
    return acc;
  }, []);
}
interface IQuotesManageProps {
  quotesData: IQuotesData;
}
function QuotesManage({ quotesData }: IQuotesManageProps) {
  ...
  useEffect(() => {
    if (quotesData.quotesArray && quotesData.quotesArray.length !== 0) {
      console.log('addId(quotesData.data) ', addId(quotesData.quotesArray));
      setState(addId(quotesData.quotesArray));
    }
  }, [quotesData.quotesArray]);
